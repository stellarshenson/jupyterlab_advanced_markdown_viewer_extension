...
- Select a passage in the preview, right-click and pick a colour under **Mark**; or select with caret browsing (F7) and press Ctrl Shift M (Cmd Shift M on macOS). Choose **Add Comment** to mark and write a note in one step. Shift Enter in the note box saves it; Enter on its own starts a new line. The first note you write asks for the initials your notes are signed with, and **Set note handle** in the command palette changes them afterwards
- Right-click a drawn Mermaid diagram and pick a colour under **Mark**, or choose **Add Comment**: the whole picture is covered in the mark's colour

...
- [x] `ACC-COPY-160` **Copy Content puts the rendered document on the clipboard as basic HTML** - MEDIUM; the context menu of a Markdown preview offers Copy Content, which puts the whole rendered document on the system clipboard in two forms at once: HTML carrying the tags and the words alone, and the same words as plain text; the HTML carries no class, no style attribute, no identifier and none of the colours the theme or this extension paint with, so a paste into a mail client arrives as headings, paragraphs, lists, tables, links, images and code and nothing else; the paragraph mark JupyterLab hangs on a heading, a control the page adds, an icon, the markers of a mark and the ghost of text a change is deleting are all left out, while a marked passage and text a change added keep their words; what a link points at, what an image shows and how far a table cell spans are kept, since they are meaning and not look; the document is not changed by the copy, and the entry is offered whether or not the marks and notes are switched on
  - evidence: src/content.ts copiedContent and the advanced-markdown-viewer:copy-content command in src/index.ts as the mechanism says; jest, the cases of describe 'copiedContent (ACC-COPY-160)' in src/**tests**/content.spec.ts, 'completes the typing on the spot when the copy asks for it (ACC-COPY-160)' and 'leaves a removal ghost in its warning hold when the copy completes the typing (ACC-COPY-160)' in src/**tests**/controller.spec.ts, and 'copies the rendered document as basic HTML and as text (ACC-COPY-160)' and 'copies the document while the marks and notes are switched off' in src/**tests**/wiring.spec.ts; Galata 'ACC-COPY-160 copies the rendered content as basic HTML, with no class, style or paint' in ui-tests/tests/copy.spec.ts failed on the installed round-60 build with no Copy Content entry in the menu (logs/galata-109-proof-acc-160.log) and is green on the round-62 to round-70 builds (logs/round70-chain.log: lint 0 errors and 12 warnings, Galata 125 passed in the notes group and 53 passed with 2 skipped in the rest group, both intermittents 3 of 3 alone; jest 709 of 709 and pytest 29 of 29 in logs/jest-109-round70.log); review rounds 13 to 19 each read the copy and each finding of theirs is closed with a jest case written first, the last two rounds clean with four SHIP verdicts each
  - test: open a preview holding a heading, a list, a link, a table and a code block, mark a passage, choose Copy Content from the preview's context menu, read the clipboard's text/html back: it holds h1, li, a with its href, table and code, no class or style attribute, no paragraph mark and none of this extension's classes; the plain text flavour holds the same words and no tags
  - test-tags: UNIT, E2E

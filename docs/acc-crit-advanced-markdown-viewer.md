...
- [ ] `ACC-ANIM-75` **Distant changes animate at the same time** - HIGH; when one write changes several paragraphs or places far apart, every added span types and every ghost deletes concurrently, not one after another; intent: a burst rewrite is followed as one event, and the animation length is set by the longest change, not by their sum
  ...
  - log: 2026-09-05T08:59:25Z @kj added
- [ ] `ACC-ANIM-76` **Animation speed is a setting** - HIGH; the setting animationSpeed sets how many characters per second the typing and the deletion play at; 0 turns the animation off and text appears and leaves at once as before; intent: the speed is tuned by the user, very fast by default
  ...
  - log: 2026-09-05T08:59:25Z @kj added
- [ ] `ACC-ANIM-77` **Reduced motion disables the typing animation** - MEDIUM; when the operating system asks for reduced motion the typing and deletion animation is off and the change lands at once, as the fades already do
  ...
  - log: 2026-09-05T08:59:25Z @kj added
- [ ] `ACC-ANIM-78` **A write during an animation continues it** - HIGH; when the next write arrives while text is still being typed, the text already typed stays on screen and only the fresh part is typed; nothing is retyped and no ghost warns twice

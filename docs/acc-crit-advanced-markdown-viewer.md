...
- [x] `ACC-NOTES-152` **Note field grows with its content** - MEDIUM; the note field starts at the height of four lines and grows as the note is typed, so every line written stays in view without a scrollbar inside the box; it shrinks back as lines are deleted and never falls below the four lines; a draft restored into the field after a panel rebuild opens at the height its lines need
  ...
  - test-tags: UNIT, E2E
  - mechanism: 2026-09-11T16:06:53Z @kj NotesPanel._form in src/notes-panel.ts: a grow function clears the field's inline height, then sets it to the field's scroll height plus its border (offsetHeight less clientHeight), so the height follows the content and the four rows of the element stay the floor because a scroll height is never below the client height; grow runs on every input event and once through a microtask after the form is built, when the field is in the document and a restored draft can be measured; style/base.css keeps the box's own scrollbar away with overflow-y hidden
...
- [x] `ACC-NOTES-153` **A click on a marked passage with notes opens its row and starts no note** - HIGH; a click on a marked passage whose mark already holds a note selects and opens the mark's row in the expanded panel, with its notes shown, and starts no new note entry; a click on a bare mark, one with no note yet, still opens the note entry as ACC-NOTES-96 says; the passage stays where it is in the view in both cases; a selection made inside marked text can still be marked, so a second mark within an existing one stays possible as ACC-NOTES-61 says
  ...
  - test-tags: UNIT, E2E
  - mechanism: 2026-09-11T16:24:37Z @kj src/index.ts: the activated handler calls NotesPanel.openFromPassage(id); the panel reads the mark from its own items and calls selectMark(id, bare, true), the second argument true only when the mark holds no note; selectMark takes a third argument fromPassage, defaulting to the second, so a click from the passage expands the panel and keeps the passage in view without scrolling it to the centre, and opens the entry only when asked; the method is not named activate, which Lumino's Widget already defines
...
- [x] `ACC-NOTES-154` **Markers inside a table row stay on one line** - HIGH; a mark placed inside a table row never breaks the table: its opening marker is written on one line even when it carries notes, with the note lines joined by a backslash and n, a pipe in a note written as a backslash and a pipe, and a backslash doubled; the opening marker is never put on a line of its own inside a table; the reader accepts both the multi-line form used everywhere else and this single-line form, reading a single-line opening marker that carries note text after its attributes as the escaped form; a file whose marks sit in table cells renders as a table with the notes listed
  - evidence: src/marks.ts, src/anchor.ts and src/notes.ts as the mechanism says; jest 'writes a marker asked inline on one line with its notes escaped, and reads it back (ACC-NOTES-154)' and 'reads the multi-line form as before, backslashes untouched' in `src/__tests__/marks.spec.ts`, 'keeps the opening marker inline on a table row, even at the first word of a cell (ACC-NOTES-154)' and 'says which offsets sit on a table row (ACC-NOTES-154)' in `src/__tests__/anchor.spec.ts`, 'writes the note on one line when the mark sits in a table row (ACC-NOTES-154)' in `src/__tests__/notes.spec.ts` (logs/jest-acc-154.log); Galata 'ACC-NOTES-154 keeps a table whole when a cell is marked and a two-line note with a pipe is written on it' in ui-tests/tests/notes.spec.ts failed on the round-47 build (logs/galata-109-proof-acc-154.log) and is green on the round-48 build of 1.0.9 with lint 0 errors, jest 627 of 627, pytest 29 of 29, Galata 118 of 118 in the notes group apart from the rejected intermittent DEF-APPLY-37-1 (2 of 3 alone, logs/galata-109-round48-forced-render-repeat-1.log to -3.log) and 49 of 49 with 2 skipped (logs/round48-chain.log)
  - test: mark two words in a table cell and add a two-line note holding a pipe: the file still renders as a table, the row holds the marker on its own line of the source with the note text escaped, the panel lists the note with its two lines and the pipe; select the first word of a cell and mark it: the marker sits inline after the cell's pipe; a marker written in the multi-line form elsewhere in the same file reads as before
  - test-tags: UNIT, E2E
  - mechanism: 2026-09-11T16:32:21Z @kj src/marks.ts: serialiseOpening takes an inline flag and, with notes, joins the note lines with backslash-n after escaping backslash, newline and pipe; readMarkers reads a single-line opening marker whose tail holds an entry head after the attributes as the escaped form, unescapes it and hands it to the same parseNotes; src/anchor.ts: inTableRow says whether an offset's line belongs to a table (a block whose second line is the separator row) and placeOpening keeps the marker inline there; src/notes.ts: the note rewrite passes inTableRow(source, mark.open.start) as the flag

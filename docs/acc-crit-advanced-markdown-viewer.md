...
- [x] `ACC-NOTES-146` **Note field font** - MEDIUM; the note field shows the text being written in the font family of the notes panel, never a monospace font, at three quarters of the panel's ordinary text size (--jp-ui-font-size1), smaller than that text
  - evidence: style/base.css .jp-AdvancedMd-notesForm textarea sets font-family: inherit and font-size: calc(var(--jp-ui-font-size1) * 0.75), 9.75 px where the panel's text is 13 px; Galata 'ACC-NOTES-146 writes the note field in the panel font at three quarters of the panel text size' failed on the round-27 build with the field at 8.12497 px against 9.75 px (logs/galata-106-proof-acc-notes-146-ordinary.log) and passes on the round-28 build of 1.0.6 (logs/galata-106-round28-notes-siblings-settings.log, 106 passed); lint 0 errors; jest 591 of 591 (logs/jest-106-round28.log)
  - test: open Add note and assert that the field's computed font family equals the panel's, holds no monospace font, that its font size is smaller than the panel's, and that it is within 0.05 px of three quarters of --jp-ui-font-size1 resolved beside the field
  - test-tags: E2E
